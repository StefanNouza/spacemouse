...
If you have mounted the magnets upside-down, the values will be inverted (increasing).

1. Try to write down the each HE sensor with the corresponding pin number you chose. If you use the PCB version, the pins are shown
   on the silk screen.
...
3. With the spacemouse in front of you (ie. USB connection at the backside ie. @ north), move the knob from south to north and observe the debug output:
  a) HES0 & HES1 both increase when moving the knob north (magnets further away) and decrease when moving the knob south -> Everything is correct.
  b) HES0 & HES1 both decrease when moving the knob south (magnets closer by) -> Everything is correct.
  c) The sensorpair acts in the opposite direction (ie. increasing when the magnets get closer by ). You probably have mixed up the poles of the magnets.
     Invert the sensorpair in the INVERTLIST.
  d) Another output is showing movement: Swap the pins in the PINLIST. This shouldn't happen when the PCB is used. Debugging is rather difficult when
...
5. Optimally when not moving the knob, all sensors should output approximately the same value. You can adjust the values a little bit
   by adjusting the height of the sensor plate using the spacernuts.

6. Repeat this with every axis and every sensor pair until you have a valid PINLIST and maybe an INVERTLIST
*/
